  ...
  var self = {
    ...
    getPrimeSupportCodeFilePaths: function getPrimeSupportCodeFilePaths() {
      ...
      supportCodeFilePaths.forEach(function(path) {
        console.log("Prime Support Code File Paths - Testing: " + path);
        if (path.match(SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP)) {
          primeSupportCodeFilePaths.push(path);
          console.log("  It matches " + SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP + " so we're going to add it.");
        }
        else {
          console.log("  It doesn't match " + SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP + " so we're NOT going to add it.");
        }
      });
    ...
    getSecondarySupportCodeFilePaths: function getSecondarySupportCodeFilePaths() {
      ...
      supportCodeFilePaths.forEach(function(path) {
        console.log("Secondary Support Code File Paths - Testing: " + path);
        if (!path.match(SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP)) {
          secondarySupportCodeFilePaths.push(path);
          console.log("  It doesn't match " + SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP + " so we're going to add it.");
        }
        else {
          console.log("  It matches " + SupportCodeLoader.PRIME_SUPPORT_CODE_PATH_REGEXP + " so we're NOT going to add it.");
        }
      });
